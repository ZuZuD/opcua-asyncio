...
def retrieve_srv_port(srv) -> str:
    return srv.endpoint.netloc.split(":")[-1]
...
def ha_config():
    ...
    def _ha_config(srv_port1, srv_port2):
        return HaConfig(
            HaMode.WARM,
            keepalive_timer=1,
            manager_timer=1,
...
async def ha_servers(server_key_and_cert):
    ...
    srv1 = await create_srv(find_free_port(), key, cert)
    srv2 = await create_srv(find_free_port(), key, cert)
    yield srv1, srv2
    # stop the servers
    for srv in srvs:
        await srv.stop()
...
async def ha_client(ha_config, ha_servers):
    ...
    srv1_port = retrieve_srv_port(srv1)
    srv2_port = retrieve_srv_port(srv2)
